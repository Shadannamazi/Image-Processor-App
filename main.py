...
Also this is where all the interactions happen
for example taking input from the users and all the functions are executed. 
"""
...
def handleUserInput(state, img):
    ...
    if userInput.isalpha(): # check if the input is an alphabet
        ...
        if userInput == "Q": # this case actually won't happen, it's here as an example
            print("Log: Quitting...")
        #Reloading without an opened image reloads the currentImg which is the black image
        elif userInput == "O":
            print("Log: Opening Image...")
            tkinter.Tk().withdraw()
            openFilename = tkinter.filedialog.askopenfilename()
            name_open = openFilename.split("/")
            ...
            if state["lastOpenFilename"]=="":
              ...
              img = cmpt120imageProjHelper.getImage(openFilename)
            cmpt120imageProjHelper.showInterface(img,"Open Image "+name_open[-1],generateMenu(state))
        
        elif userInput == "S":
            print("Log: Saving Image...") 
            tkinter.Tk().withdraw()
            saveFilename = tkinter.filedialog.asksaveasfilename()
            name_save = saveFilename.split("/")
            state["lastSaveFilename"] = saveFilename
            cmpt120imageProjHelper.saveImage(img,saveFilename)
            cmpt120imageProjHelper.showInterface(img,"Saved Image "+name_save[-1],generateMenu(state))
